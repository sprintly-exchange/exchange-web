...
const SftpDelivery = () => {
  ...
  async function sendData(url,data){
    ...
    try{
     ...
     setLoading(false);
     message.info(`A SFTP delivery was created with ID ${responseData.id}.`);
    }catch(error){
  ...
  return (
    <>
    <div>
      <Form  name ="formStpDelivery" form={form} layout="horizontal" labelCol={{ span: 8, }} wrapperCol={{ span: 16, }} style={{ maxWidth: 600, }} initialValues={{ remember: true, }} onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete="on">
