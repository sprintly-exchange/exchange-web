import React, { useState, useEffect } from 'react';
import axiosInstance from '../utils/axiosConfig';
import configManagerFE from '../configuration/configManager';
import CurrentUsersTable from './CurrentUsersTable';
import { Form, Input, Button, Select, message, Typography, Row, Col } from 'antd';
import countryList from 'react-select-country-list'; // List of countries with codes and flags

const { Text } = Typography;
const { Option } = Select;

// Map of ISO country codes to numeric country codes
const countryCodeMap = {
  AF: "+93",   AL: "+355",  DZ: "+213",  AS: "+1-684",  AD: "+376",
  AO: "+244",  AI: "+1-264",  AG: "+1-268",  AR: "+54",  AM: "+374",
  AW: "+297",  AU: "+61",   AT: "+43",   AZ: "+994",  BS: "+1-242",
  BH: "+973",  BD: "+880",  BB: "+1-246",  BY: "+375",  BE: "+32",
  BZ: "+501",  BJ: "+229",  BM: "+1-441",  BT: "+975",  BO: "+591",
  BA: "+387",  BW: "+267",  BR: "+55",   IO: "+246",  VG: "+1-284",
  BN: "+673",  BG: "+359",  BF: "+226",  BI: "+257",  KH: "+855",
  CM: "+237",  CA: "+1",    CV: "+238",  KY: "+1-345",  CF: "+236",
  TD: "+235",  CL: "+56",   CN: "+86",   CX: "+61",   CC: "+61",
  CO: "+57",   KM: "+269",  CK: "+682",  CR: "+506",  HR: "+385",
  CU: "+53",   CW: "+599",  CY: "+357",  CZ: "+420",  CD: "+243",
  DK: "+45",   DJ: "+253",  DM: "+1-767",  DO: "+1-809",  EC: "+593",
  EG: "+20",   SV: "+503",  GQ: "+240",  ER: "+291",  EE: "+372",
  SZ: "+268",  ET: "+251",  FJ: "+679",  FI: "+358",  FR: "+33",
  GF: "+594",  PF: "+689",  GA: "+241",  GM: "+220",  GE: "+995",
  DE: "+49",   GH: "+233",  GI: "+350",  GR: "+30",   GL: "+299",
  GD: "+1-473",  GP: "+590",  GU: "+1-671",  GT: "+502",  GN: "+224",
  GW: "+245",  GY: "+592",  HT: "+509",  HN: "+504",  HK: "+852",
  HU: "+36",   IS: "+354",  IN: "+91",   ID: "+62",   IR: "+98",
  IQ: "+964",  IE: "+353",  IL: "+972",  IT: "+39",   CI: "+225",
  JM: "+1-876",  JP: "+81",   JO: "+962",  KZ: "+7",    KE: "+254",
  KI: "+686",  KW: "+965",  KG: "+996",  LA: "+856",  LV: "+371",
  LB: "+961",  LS: "+266",  LR: "+231",  LY: "+218",  LI: "+423",
  LT: "+370",  LU: "+352",  MO: "+853",  MK: "+389",  MG: "+261",
  MW: "+265",  MY: "+60",   MV: "+960",  ML: "+223",  MT: "+356",
  MH: "+692",  MQ: "+596",  MR: "+222",  MU: "+230",  YT: "+262",
  MX: "+52",   FM: "+691",  MD: "+373",  MC: "+377",  MN: "+976",
  ME: "+382",  MS: "+1-664",  MA: "+212",  MZ: "+258",  MM: "+95",
  NA: "+264",  NR: "+674",  NP: "+977",  NL: "+31",   NC: "+687",
  NZ: "+64",   NI: "+505",  NE: "+227",  NG: "+234",  NU: "+683",
  NF: "+672",  KP: "+850",  MP: "+1-670",  NO: "+47",   OM: "+968",
  PK: "+92",   PW: "+680",  PS: "+970",  PA: "+507",  PG: "+675",
  PY: "+595",  PE: "+51",   PH: "+63",   PL: "+48",   PT: "+351",
  PR: "+1-787",  QA: "+974",  RE: "+262",  RO: "+40",   RU: "+7",
  RW: "+250",  BL: "+590",  SH: "+290",  KN: "+1-869",  LC: "+1-758",
  MF: "+590",  PM: "+508",  VC: "+1-784",  WS: "+685",  SM: "+378",
  ST: "+239",  SA: "+966",  SN: "+221",  RS: "+381",  SC: "+248",
  SL: "+232",  SG: "+65",   SX: "+1-721",  SK: "+421",  SI: "+386",
  SB: "+677",  SO: "+252",  ZA: "+27",   KR: "+82",   SS: "+211",
  ES: "+34",   LK: "+94",   SD: "+249",  SR: "+597",  SE: "+46",
  CH: "+41",   SY: "+963",  TW: "+886",  TJ: "+992",  TZ: "+255",
  TH: "+66",   TL: "+670",  TG: "+228",  TO: "+676",  TT: "+1-868",
  TN: "+216",  TR: "+90",   TM: "+993",  TC: "+1-649",  TV: "+688",
  UG: "+256",  UA: "+380",  AE: "+971",  GB: "+44",   US: "+1",
  UY: "+598",  UZ: "+998",  VU: "+678",  VA: "+379",  VE: "+58",
  VN: "+84",   WF: "+681",  EH: "+212",  YE: "+967",  ZM: "+260",
  ZW: "+263"
};


function UserManagement() {
  const [users, setUsers] = useState([]);
  const [organizations, setOrganizations] = useState([]);
  const [selectedOrganizationId, setSelectedOrganizationId] = useState('');
  const [roles, setRoles] = useState([]);
  const [selectedRoleId, setSelectedRoleId] = useState('');
  const [countryCode, setCountryCode] = useState('+1'); // Default country code
  const [phoneNumber, setPhoneNumber] = useState('');
  const countryOptions = countryList().getData(); // Get country data

  useEffect(() => {
    fetchOrganizations();
    fetchUsers();
    fetchRoles();
  }, []);

  async function fetchOrganizations() {
    try {
      axiosInstance.get(`/api/organizations`)
        .then((response) => {
          setOrganizations(response.data);
        })
        .catch((error) => {
          setOrganizations([]);
        });

    } catch (error) {
      console.error('Error fetching organizations:', error);
    }
  }

  async function fetchRoles() {
    axiosInstance.get(`/api/users/roles`)
      .then((response) => {
        setRoles(response.data);
      })
      .catch((error) => {
        console.error('Error fetching roles:', error);
      });
  }

  async function fetchUsers() {
    try {
      axiosInstance.get(`/api/users`)
        .then((response) => {
          const data = response.data;
          setUsers(Array.isArray(data) ? data : [data]);
        })
        .catch((error) => {
          setUsers([]);
        });

    } catch (error) {
      console.error('Error fetching users:', error);
    }
  }

  async function addUser(username, password, email, fullPhoneNumber, organizationId, roleId) {
    try {
      await axiosInstance.post(
        `/api/users/register-user`,
        { username, password, email, mobileNumber: fullPhoneNumber, organizationId, roleId }
      )
        .then((response) => {
          message.info(response.data.message);
        })
        .catch((error) => {
          message.warning(error.response.data.message);
        })
      fetchUsers();
    } catch (error) {
      // Handle the error message from the server
      console.error('Error registering user:', error);
    }
  }

  const handleUserSubmit = (values) => {
    const fullPhoneNumber = `${countryCode} ${phoneNumber}`;
    addUser(values.username, values.password, values.email, fullPhoneNumber, selectedOrganizationId, selectedRoleId);
  };

  const validateUsername = (_, value) => {
    if (value && /\s/.test(value)) {
      return Promise.reject('Username cannot contain spaces');
    }
    return Promise.resolve();
  };

  return (
    <div>
      <Typography.Title level={3}>User Management</Typography.Title>
      <div style={{ padding: '20px', width: '50%' }}>
        <Form onFinish={handleUserSubmit}>
          <Form.Item name="username" label="User Name" rules={[{ required: true, message: 'Please input the username!' }, { validator: validateUsername }]}>
            <Input />
          </Form.Item>
          <Form.Item name="password" label="Password" rules={[{ required: true, message: 'Please input the password!' }]}>
            <Input.Password />
          </Form.Item>
          <Form.Item name="email" label="Email" rules={[{ required: true, message: 'Please input the email!', type: 'email' }]}>
            <Input />
          </Form.Item>
          <Form.Item label="Mobile Number" required>
            <Row gutter={8}>
              <Col span={8}>
                <Select
                  showSearch
                  value={countryCode}
                  onChange={(value) => setCountryCode(value)}
                  style={{ width: '100%' }}
                  placeholder="Country Code"
                >
                  {countryOptions.map(country => {
                    const numericCode = countryCodeMap[country.value] || '';
                    return (
                      <Option key={country.value} value={numericCode}>
                        <img src={`https://flagcdn.com/w20/${country.value.toLowerCase()}.png`} alt={country.label} style={{ marginRight: 8 }} />
                        {country.label} ({numericCode})
                      </Option>
                    );
                  })}
                </Select>
              </Col>
              <Col span={16}>
                <Input
                  value={phoneNumber}
                  onChange={(e) => setPhoneNumber(e.target.value)}
                  placeholder="Phone Number"
                />
              </Col>
            </Row>
          </Form.Item>
          <Form.Item label="Organization">
            <Select onChange={(value) => setSelectedOrganizationId(value)} value={selectedOrganizationId}>
              {(Array.isArray(organizations) ? organizations : []).map(org => (
                <Option key={org.id} value={org.id}>{org.name}</Option>
              ))}
            </Select>
          </Form.Item>
          <Form.Item label="Role">
            <Select onChange={(value) => setSelectedRoleId(value)} value={selectedRoleId}>
              {(Array.isArray(roles) ? roles : []).map(role => (
                <Option key={role.id} value={role.id}>{role.role}</Option>
              ))}
            </Select>
          </Form.Item>
          <Form.Item>
            <Button type="primary" htmlType="submit">
              Create User
            </Button>
          </Form.Item>
        </Form>
      </div>
      <div>
        <CurrentUsersTable />
      </div>
    </div>
  );
}

export default UserManagement;
