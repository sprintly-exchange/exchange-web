...
import {ConnectionFTP} from '../../models/ConnectionFTP.mjs'
...
const FtpPickup = () => {
  ...
  const onFinish = (values) => {
    ...
    sendData(`${process.env.REACT_APP_API_BASE_URL}/api/pickup`,connectionFTP);
  ...
  async function sendData(url,data){
    ...
    try{
     ...
     setLoading(false);
     message.info(`A FTP pickup was created with ID ${responseData.id}.`);
    }catch(error){
  ...
  return (
    <>
    <div>
      <Form  name ="formFtpPickup" form={form} layout="horizontal" labelCol={{ span: 8, }} wrapperCol={{ span: 16, }} style={{ maxWidth: 600, }} initialValues={{ remember: true, }} onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete="on">
